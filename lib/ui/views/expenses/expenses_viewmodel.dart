...
class ExpensesViewModel extends BaseViewModel with NavigationMixin {
  ExpensesViewModel() {
    initialState();
    getDateUsers();
  }
  late final List<ExpensesModel> _userList = [];
  ...
  List<ExpensesModel> get userList => _userList ?? [];
  ...
  Future<void> getAllDonationUserDetails() async {
    ...
    users.forEach((user) {
      ...
      userModel.purpose = user['purpose'];
  ...
  Future<void> getDateUsers() async {
    _userList.clear();
    _totalAmount = 0.0;
    var users = await _userService.readYearExpenses(_selectedYear);
  ...
  void addForm() {
    String donation = "Expenses";
    goToAddForm(donation);
    notifyListeners();
  }

  void deleteDatas(id) async {
    var result = await _userService.deleteUserExpenses(id);
    await getDateUsers();
    notifyListeners();
  }
  ...
  Future<void> importFile(String filePath, bool isCSV) async {
    ...
    for (var row in data.skip(1)) {
      ...
      var purpose = row[0]?.value?.toString() ?? '';
      var item = row[1]?.value?.toString() ?? '';
      var amount = double.tryParse(row[2]?.value?.toString() ?? '0');
      ...
      if (purpose.isNotEmpty && amount != null) {
        var user = ExpensesModel();
    ...
    await userService.saveExpenses(users);
    getDateUsers();
  ...
  void performSearch() async {
    ...
    if (query.isNotEmpty) {
      ...
      results.forEach((user) {
        ...
        var userModel = ExpensesModel();
        userModel.id = user['id'];
        userModel.purpose = user['name'];
        userModel.item = user['area'];
